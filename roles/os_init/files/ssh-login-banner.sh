...
totaldisk=$(df -h -x aufs -x tmpfs -x overlay -x devtmpfs -x shm --total 2>/dev/null | tail -1)
...
MEM_INFO="$(free -h  | grep Mem | awk '{print $7}') (available) / $(free -h  | grep Mem | awk '{print $2}')"
